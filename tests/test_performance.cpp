...
class PerformanceTest : public ::testing::Test {
 protected:
  ...
  void cleanup() { std::filesystem::remove_all(base_path); }
...
TEST_F(PerformanceTest, WriteAndRead) {
  ...
  std::cout << "每秒操作数 (TPS): "
            << static_cast<long long>(num_operations / write_duration.count())
            << std::endl;
  std::cout << "------------------------------------" << std::endl;
  ...
  std::cout << "每秒操作数 (QPS): "
            << static_cast<long long>(num_operations / read_duration.count())
            << std::endl;
  std::cout << "------------------------------------" << std::endl;
}
