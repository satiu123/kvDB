...
std::vector<std::uint8_t> WalRecord::serialize() const {
    ...
    // 前置总长度
    std::uint32_t total_size = static_cast<std::uint32_t>(data.size() + sizeof(std::uint32_t));
...
std::expected<std::unique_ptr<WalRecord>, std::string> WalRecord::deserialize(
    ...
    if (!total_size_res) return std::unexpected("Failed to read total size: " + total_size_res.error());
    if (*total_size_res != size) return std::unexpected("Record size mismatch");

    // 2. 读取并校验CRC
    auto stored_crc_res = kvdb::core::binary::read_uint32(stream);
    if (!stored_crc_res) return std::unexpected("Failed to read checksum: " + stored_crc_res.error());
    ...
    if (!op_type_res) return std::unexpected(op_type_res.error());
    ...
    return std::make_unique<WalRecord>(static_cast<WalOpType>(*op_type_res), *key_res, *value_res, *seq_num_res);
}

// 获取记录序列化后大小
std::size_t WalRecord::size() const {
    // Payload size = op_type + key_with_len + value_with_len + sequence_number
    std::size_t payload_size = sizeof(std::uint8_t) + 
                             (sizeof(std::uint32_t) + key_.size()) + 
                             (sizeof(std::uint32_t) + value_.size()) + 
                             sizeof(std::uint64_t);
...
static std::string op_type_to_string(WalOpType op_type) {
    switch (op_type) {
        case WalOpType::PUT:    return "PUT";
        case WalOpType::REMOVE: return "REMOVE";
        case WalOpType::CLEAR:  return "CLEAR";
