...
bool Snapshot::create(const std::unordered_map<std::string, std::string>& data,
    ...
    LOG_INFO("快照创建成功: {}, 记录数: {}, WAL偏移: {}", snapshot_path_, data.size(), wal_offset);
    return true;
}

bool Snapshot::restore(std::unordered_map<std::string, std::string>& data, uint64_t& wal_offset) {
    ...
    LOG_INFO("快照恢复成功: {}, 记录数: {}, WAL偏移: {}", snapshot_path_, data.size(), wal_offset);
    return true;
}
